...
(function () {
  ...
  function boot() {
    ...
    var RENDERERS = {
      ...
      /* ---- hub: composite home — hero, stats, about, zones, featured band, more ---- */
      hub: function (p) {
        var lang = L.state.lang, en = lang === "en";
        var h = p.hero || {};

        function btn(b, cls, ext) {
          if (!b) return "";
          var attrs = ext ? ' target="_blank" rel="noopener"' : "";
          return '<a class="btn ' + cls + '" href="' + esc(b.href) + '"' + attrs + '>' +
            esc(t(b.label)) +
            (ext ? '<span class="material-symbols-rounded" aria-hidden="true">open_in_new</span>'
                 : '<span class="material-symbols-rounded" aria-hidden="true">arrow_forward</span>') +
            "</a>";
        }

        var hero =
          '<header class="hero" data-item>' +
            '<div class="hero__copy">' +
              (t(h.tagline) ? '<span class="hero__eyebrow">' + esc(t(h.tagline)) + "</span>" : "") +
              '<h1 class="hero__title">' + esc(t(p.title)) + "</h1>" +
              '<p class="hero__lead">' + esc(t(p.subtitle)) + "</p>" +
              '<div class="hero__facts">' +
                '<p class="hero__fact"><span class="material-symbols-rounded" aria-hidden="true">event</span>' + esc(t(h.date)) + "</p>" +
                '<p class="hero__fact"><span class="material-symbols-rounded" aria-hidden="true">location_on</span>' +
                  esc(t(h.location)) + (t(h.address) ? " · " + esc(t(h.address)) : "") + "</p>" +
              "</div>" +
              '<div class="hero__cta">' +
                btn(h.ctaPrimary, "btn--filled") +
                btn(h.ctaSecondary, "btn--tonal") +
                btn(h.official, "btn--text", true) +
              "</div>" +
            "</div>" +
          "</header>";
        ...
        stats = stats ? '<div class="hero__stats">' + stats + "</div>" : "";

        var about = p.about ? '<section class="home-section" data-item>' +
          '<h2 class="home-section__title">' + esc(t(p.about.heading)) + "</h2>" +
          '<p class="home-section__lead">' + esc(t(p.about.body)) + "</p></section>" : "";

        var zoneCards = (p.zones || []).map(function (z) {
          var cnt = z.count ? '<span class="zone-card__count">' + esc(String(z.count)) +
            (en ? " exhibitors" : " 家") + "</span>" : "";
          return '<a class="zone-card" data-item href="exhibitors.html#zone=' + esc(z.cat || "") + '">' +
            '<span class="material-symbols-rounded zone-card__icon" aria-hidden="true">' + esc(z.icon || "label") + "</span>" +
            '<div class="zone-card__body"><h3 class="zone-card__name">' + esc(t(z.name)) + "</h3>" +
            '<p class="zone-card__tag">' + esc(t(z.tagline)) + "</p></div>" + cnt + "</a>";
        }).join("");
        var zones = (p.zones && p.zones.length) ? '<section class="home-section" data-item>' +
          '<h2 class="home-section__title">' + esc(t(p.zonesHeading)) + "</h2>" +
          '<div class="zone-grid">' + zoneCards + "</div></section>" : "";

        var f = p.featured;
        var featured = f ? '<section class="featured-band" data-item>' +
          '<div class="featured-band__copy">' +
            (t(f.eyebrow) ? '<span class="featured-band__eyebrow">' + esc(t(f.eyebrow)) + "</span>" : "") +
            '<h2 class="featured-band__title">' + esc(t(f.heading)) + "</h2>" +
            '<p class="featured-band__body">' + esc(t(f.body)) + "</p>" +
            '<a class="btn btn--filled" href="' + esc(f.href) + '">' + esc(t(f.cta)) +
              (f.count ? ' <span class="btn__badge">' + esc(String(f.count)) + "</span>" : "") +
              ' <span class="material-symbols-rounded" aria-hidden="true">arrow_forward</span></a>' +
          "</div>" +
          '<span class="material-symbols-rounded featured-band__art" aria-hidden="true">clinical_notes</span>' +
        "</section>" : "";

        var skip = { home: 1, exhibitors: 1, "medical-ai": 1 };
        var moreCards = L.pages.filter(function (q) { return !skip[q.slug]; }).map(function (q) {
        ...
        var more = moreCards ? '<section class="home-section" data-item>' +
          '<h2 class="home-section__title">' + (en ? "Explore more" : "更多") + "</h2>" +
          '<div class="grid">' + moreCards + "</div></section>" : "";

        return hero + stats + about + zones + featured + more;
      },
      ...
      article: function (p) {
        ...
        var body = (p.sections || []).map(function (s) {
          var blocks = (s.blocks || []).map(function (b) {
            ...
            if (b.type === "links") {
              var links = (b.items || []).map(function (lk) {
                return '<li><a class="article-link" href="' + esc(lk.href) + '" target="_blank" rel="noopener">' +
                  '<span class="material-symbols-rounded" aria-hidden="true">open_in_new</span>' +
                  esc(t(lk.label)) + "</a></li>";
              }).join("");
              return '<ul class="article-links">' + links + "</ul>";
            }
      ...
      /* ---- glossary: searchable term list (native <details>) ---- */
      glossary: function (p) {
        var items = (p.terms || []).map(function (row) {
          var hay = (t(row.term) + " " + t(row.def)).toLowerCase();
          return '<details class="acc-item gl-item" data-item data-q="' + esc(hay) + '">' +
            '<summary class="acc-q"><span class="gl-term">' + esc(t(row.term)) + "</span>" +
            '<span class="material-symbols-rounded acc-chevron" aria-hidden="true">expand_more</span></summary>' +
        ...
        return head(p) +
          ...
          '<p class="result-count" id="resultCount" aria-live="polite"></p>' +
          '<div class="accordion" id="glossary">' + items + "</div>";
      },
      ...
      quiz: function (p) {
        ...
        var qs = (p.questions || []).map(function (q, qi) {
          ...
          return '<li class="quiz-card" data-item data-qi="' + qi + '">' +
            '<p class="quiz-q"><span class="quiz-q__num">' + (qi + 1) + "/" + n + "</span>" + esc(t(q.q)) + "</p>" +
      ...
      /* ---- flashcards: click to flip + prev/next (ephemeral) ---- */
      flashcards: function (p) {
        var n = (p.cards || []).length;
        var front = L.state.lang === "en" ? "Tap to flip" : "點擊翻面";
        var cards = (p.cards || []).map(function (c, i) {
          return '<button class="flip" type="button" data-item data-i="' + i + '" aria-pressed="false">' +
            '<span class="flip__inner">' +
              '<span class="flip__face flip__front"><span class="flip__hint">' + esc(front) + "</span>" +
                '<span class="flip__text">' + esc(t(c.front)) + "</span></span>" +
              '<span class="flip__face flip__back"><span class="flip__text">' + esc(t(c.back)) + "</span></span>" +
            "</span></button>";
        }).join("");
        return head(p) +
          '<p class="result-count">' + n + (L.state.lang === "en" ? " cards · tap a card to reveal" : " 張卡片 · 點卡片看解答") + "</p>" +
    ...
    var WIRE = {
      ...
      gallery: function (p) {
        ...
        /* both-language text so search works regardless of the active language */
        function both(o) { return o == null ? "" : (typeof o === "string" ? o : ((o.en || "") + " " + (o.zh || ""))); }
        function tagHtml(g) { return '<span class="tag">' + esc(t(g)) + "</span>"; }
        function tagsHtml(item) {
          return (item.tags || []).map(tagHtml).join("");
        }
        function logoHtml(item, cls) {
          if (item.logo) {
            return '<img class="' + cls + '" src="' + esc(item.logo) + '" alt="' + esc(t(item.title)) +
              '" loading="lazy" />';
          }
          var ch = (t(item.title) || "?").trim().charAt(0);
          return '<span class="' + cls + ' ' + cls + '--mono" aria-hidden="true">' + esc(ch) + "</span>";
        }

        function matches(item) {
          if (st.cat && item.category !== st.cat) return false;
          if (!st.q) return true;
          var hl = (item.highlights || []).map(both).join(" ");
          var tg = (item.tags || []).map(both).join(" ");
          var hay = (both(item.title) + " " + both(item.summary) + " " + tg + " " + hl).toLowerCase();
        ...
        function paint() {
          ...
          grid.innerHTML = rows.map(function (item) {
            ...
            return '<article class="card card--vendor" tabindex="0" role="button" data-item data-slug="' + esc(item.slug) + '" ' +
              'aria-label="' + esc(t(item.title)) + '">' +
              '<div class="card__logo-wrap">' + logoHtml(item, "card__logo") + "</div>" +
        ...
        function openItem(slug) {
          ...
          var tags = tagsHtml(item);
          var hl = (item.highlights || []).map(function (g) {
            return "<li>" + esc(t(g)).replace(/\n\n/g, "<br><br>") + "</li>";
          }).join("");
          var visit = item.url ? '<a class="btn btn--filled vendor-modal__visit" href="' + esc(item.url) +
            '" target="_blank" rel="noopener">' + (L.state.lang === "en" ? "Visit website" : "前往官網") +
            '<span class="material-symbols-rounded" aria-hidden="true">open_in_new</span></a>' : "";
          body.innerHTML =
            '<div class="vendor-modal__head">' +
              '<div class="vendor-modal__logo-wrap">' + logoHtml(item, "vendor-modal__logo") + "</div>" +
              '<div class="vendor-modal__heading"><h2 id="dialogTitle">' + esc(t(item.title)) + "</h2>" +
                (tags ? '<div class="card__tags">' + tags + "</div>" : "") + "</div>" +
            "</div>" +
            (hl ? '<h3 class="vendor-modal__sub">' + (L.state.lang === "en" ? "Highlights" : "展出亮點") + "</h3>" +
                  '<ul class="vendor-modal__highlights">' + hl + "</ul>" : "") +
            visit;
        ...
        function applyCat(cat) {
          ...
          chips.forEach(function (c) {
            c.classList.toggle("chip--active", (c.dataset.cat || "") === st.cat);
          });
          paint();
        }
        function syncHash() {
          ...
          if (!hash) return;
          if (hash.indexOf("zone=") === 0) { applyCat(hash.slice(5)); return; }
          if (findItem(hash)) openItem(hash);
        }
      ...
      glossary: function () {
        ...
        if (search) search.addEventListener("input", function () { tally(this.value.trim().toLowerCase()); });
        tally("");
      },
      ...
      quiz: function (p) {
        ...
        var answered = {};   // qi -> true ; session-only, resets on reload/lang switch
        var correct = 0;
        function refreshScore() {
          var done = Object.keys(answered).length;
          var msg = L.state.lang === "en"
            ? ("Score " + correct + " / " + n + "  ·  answered " + done + "/" + n)
            : ("得分 " + correct + " / " + n + "  ·  已作答 " + done + "/" + n);
        ...
        [].forEach.call(pageEl.querySelectorAll(".quiz-card"), function (card) {
          ...
          opts.forEach(function (btn) {
            btn.addEventListener("click", function () {
              if (answered[qi]) return;            // lock after first answer
              answered[qi] = true;
              ...
              if (explain) {
                explain.textContent = (right ? "✅ " : "❌ ") + t(q.explain);
      ...
      flashcards: function () {
        [].forEach.call(pageEl.querySelectorAll(".flip"), function (btn) {
          btn.addEventListener("click", function () {
            var on = btn.classList.toggle("flip--on");
